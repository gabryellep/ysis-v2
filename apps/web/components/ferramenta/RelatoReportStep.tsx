...
export function RelatoReportStep({ relato, purpose, discreetMode, onBack, onEditRelato, onStartNew }: RelatoReportStepProps) {
      ...
      <section className="grid flex-1 gap-5 xl:grid-cols-[0.72fr_1.28fr]">
        <div className="rounded-[2rem] border border-[rgba(103,43,66,0.08)] bg-[linear-gradient(135deg,rgba(255,251,246,0.94),rgba(231,176,184,0.18),rgba(188,167,219,0.14))] p-6 shadow-[0_16px_48px_rgba(103,43,66,0.055)]">
          ...
          <p className="mt-4 text-sm leading-7 text-muted">
            Revise o conteudo antes de usar. A Ysis organiza o texto, mas nao interpreta sintomas como diagnostico.
          </p>

          <div className="mt-5 space-y-2 rounded-2xl bg-white/52 p-4 text-xs leading-5 text-muted">
            <p>Este material nao substitui atendimento profissional.</p>
            <p>Voce controla o que deseja manter, editar, copiar ou apagar.</p>
            <p>Se houver dor intensa, sangramento importante, febre, desmaio ou outro sinal urgente, procure atendimento de saude.</p>
          </div>
          ...
          <div className="mt-6 space-y-3">
            <button type="button" onClick={copyReport} className="w-full rounded-xl bg-[rgb(var(--color-wine))] px-4 py-3 text-sm font-semibold text-paper transition hover:bg-[rgba(103,43,66,0.9)]">
              {copyState === "copied" ? (discreetMode ? "Documento copiado" : "Relatorio copiado") : copyState === "failed" ? "Nao foi possivel copiar" : discreetMode ? "Copiar documento" : "Copiar relatorio"}
            </button>
        ...
        <article className="rounded-[2rem] border border-[rgba(103,43,66,0.08)] bg-[rgb(var(--color-paper))] p-5 shadow-[0_16px_48px_rgba(103,43,66,0.055)] lg:p-7">
          ...
          <ReportSection title={discreetMode ? "Pontos ou situacoes descritas" : "Sintomas ou situacoes relatadas"} items={report.reportedSituations} />
